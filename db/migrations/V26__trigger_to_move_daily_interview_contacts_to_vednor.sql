...
BEGIN
    ...
    -- Normalize LinkedIn ID for matching
    IF p_linkedin IS NOT NULL AND p_linkedin != '' THEN
        SET p_linkedin = REGEXP_REPLACE(p_linkedin, '^.*linkedin\\.com/in/([^/?#]+).*$','$1');
    END IF;

    -- Search using OR logic - if ANY field matches, return that vendor
...
BEGIN
    ...
    -- Extract LinkedIn ID
    SET v_linkedin_id = NEW.interviewer_linkedin;
    IF v_linkedin_id IS NOT NULL AND v_linkedin_id != '' THEN
        SET v_linkedin_id = REGEXP_REPLACE(v_linkedin_id, '^.*linkedin\\.com/in/([^/?#]+).*$','$1');
    END IF;
    ...
    ELSE
        ...
        SET 
            ...
            linkedin_id = COALESCE(v_linkedin_id, linkedin_id),
...
BEGIN
    ...
    IF v_has_changes THEN
        -- Extract LinkedIn ID from new or old value
        SET v_linkedin_id = COALESCE(NEW.interviewer_linkedin, OLD.interviewer_linkedin);
        IF v_linkedin_id IS NOT NULL AND v_linkedin_id != '' THEN
            SET v_linkedin_id = REGEXP_REPLACE(v_linkedin_id, '^.*linkedin\\.com/in/([^/?#]+).*$','$1');
        END IF;
