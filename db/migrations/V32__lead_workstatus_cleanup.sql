-- V32__lead_workstatus_cleanup.sql
...
-- If table exists, perform cleanup
IF @lead_exists > 0 THEN

    -- Audit invalid workstatus values
    ...
    WHERE workstatus IN ('CITIZEN','US CITIZEN','USC') AND id > 0;

    UPDATE `lead`
    SET workstatus = 'GREEN_CARD'
    WHERE workstatus IN ('GREEN CARD','PERMANENT RESIDENT') AND id > 0;

    UPDATE `lead`
    SET workstatus = 'GC_EAD'
    WHERE workstatus IN ('GC','GC EAD','EAD') AND id > 0;

    UPDATE `lead`
    SET workstatus = 'F1_OPT'
    WHERE workstatus = 'OPT' AND id > 0;

    UPDATE `lead`
    SET workstatus = 'H4'
    WHERE workstatus IN ('H4','H4 (WAITING FOR EAD)') AND id > 0;

    UPDATE `lead`
    SET workstatus = 'H4_EAD'
    WHERE workstatus IN ('H4EAD','H4 EAD') AND id > 0;

    UPDATE `lead`
    SET workstatus = 'L1A'
    WHERE workstatus = 'L1' AND id > 0;

    UPDATE `lead`
    SET workstatus = 'L2'
    WHERE workstatus = 'L2' AND id > 0;

    UPDATE `lead`
    SET workstatus = NULL
    WHERE workstatus IN (
      'WAITING','WAITING FOR STATUS','VISA','OTHER','NON-US','INDIA'
    ) AND id > 0;

    -- Audit remaining workstatus
    SELECT DISTINCT workstatus FROM `lead`;

    -- Clean empty or whitespace workstatus
    ...
    SELECT COUNT(*) AS invalid_closed
    FROM `lead`
