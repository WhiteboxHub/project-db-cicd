...
BEGIN
  IF NEW.email IS NOT NULL AND NEW.email <> '' THEN
    INSERT INTO outreach_contacts (email, source_type, source_id)
    VALUES (NEW.email, 'company_hr_contacts', NEW.id)
...
BEGIN
  IF NEW.email IS NOT NULL AND NEW.email <> '' THEN
    INSERT INTO outreach_contacts (email, source_type, source_id)
    VALUES (NEW.email, 'vendor_contact_extracts', NEW.id)
    ON DUPLICATE KEY UPDATE
      updated_at = CURRENT_TIMESTAMP;
  END IF;
END$$
...
BEGIN
  IF NEW.email IS NOT NULL AND NEW.email <> '' THEN
    INSERT INTO outreach_contacts (email, source_type, source_id)
    VALUES (NEW.email, 'vendor', NEW.id)
    ON DUPLICATE KEY UPDATE
      updated_at = CURRENT_TIMESTAMP;
  END IF;
END$$
