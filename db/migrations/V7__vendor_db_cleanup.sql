...
-- Preview queries below should NOT be run as migration statements in Flyway;
-- Only DML (Data Manipulation Language, like DELETE) should be.
-- If you want to preview data, run these SELECT queries manually in your SQL client.

-- The following preview query helps identify duplicates:
-- SELECT *
-- FROM vendor
-- WHERE email IN (
--     SELECT email
...
-- The following preview query shows a score for each row with a duplicate email:
-- SELECT
--     v.*,
--     (
...
-- Actual DML: Delete lower-quality duplicate vendor rows,
-- keeping only the highest-scoring per email (by data_score, then most recent).
DELETE v
FROM vendor v
JOIN (
    SELECT id
